...
for subset_name in list(subset_ht_dic.keys()):
    ...
    tstart = time.time()
    subset_ht_dic[subset_name].write('{out_prefix}.{subset_name}.ht'.format(out_prefix = args.output_prefix, subset_name = subset_name))
    tend = time.time()
    logging.info('--> Writing {} to disk FINISHED! {} seconds elapsed'.format(subset_name, tend - tstart))
subset_name = 'covariates'
logging.info('--> Writing {} to disk'.format(subset_name))
tstart = time.time()
covar.write('{out_prefix}.{subset_name}.ht'.format(out_prefix = args.output_prefix, subset_name = subset_name))
tend = time.time()
logging.info('--> Writing {} to disk FINISHED! {} seconds elapsed'.format(subset_name, tend - tstart))
